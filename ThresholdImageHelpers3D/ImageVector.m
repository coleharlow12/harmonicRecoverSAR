...
%% Script
function [I,dRnd] = ImageVector(E,txLoc,rxLoc,lambda,nCells,ix,iy,iz)
    ...
    k1 = 2*pi./(lambda); %dimensions of NF x 1
%     k = permute(k1,[3,2,1]); %dimensions of 1 x 1 x NF
...
%     cField = 4*pi*repmat(dRnd,1,1,NF).*cField; %Scales field by the distance travelled
%     %cField = 4*pi*cField;
%     I = squeeze(sum(sum((permute(cField,[3,1,2]).*repmat(E,1,1,NC)),1),2));
%     I = reshape((I),nCells);
%     %dRnd = reshape(squeeze(dRnd),nCells);
    
    %% Faster Method (Uses Less Memory
    I = zeros(size(dRnd));
    
    for i=1:length(k1)
        phsChange = 1i*dRnd*k1(i);
        phsChange = exp(phsChange); % Has dimensions of NR x NC
        I = I + E(i,:)*phsChange;
    end
    I = sum(I,1);
    I = reshape((I),nCells);
